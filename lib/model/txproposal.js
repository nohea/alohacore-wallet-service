...
TxProposal.create = function(opts) {
  ...
  console.log("  Constants.NETWORKS %s, x.network %s", Constants.NETWORKS, x.network);
  $.checkState(_.contains(_.values(Constants.NETWORKS), x.network));

  x.setInputs(opts.inputs);
  //console.log("  self.fee = opts.fee ", opts.fee);
  x.fee = opts.fee;
...
TxProposal.prototype._buildTx = function() {
  //console.log("TxProposal._buildTx() ");
  var self = this;
  ...
  // Peercoin pre-v0.7
  if(self.timestamp) {
    //console.log("TxProposal._buildTx() self.timestamp: ", self.timestamp);
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      ...
      //console.log("  add output addr %s for %d satoshis (%s)", o.toAddress, o.amount, o.amount / Defaults.COIN );
      t.to(o.toAddress, o.amount);
    }
  });

  //console.log("  fee: %s", self.fee );
  t.fee(self.fee);

  if (self.changeAddress) {
    //console.log("  change addr: %s", self.changeAddress.address);
    t.change(self.changeAddress.address);
  }
  else {
    //console.log("  no change addr");
  }
  ...
  //console.log("  checkState() totalInputs %s >  && totalOutputs %s > 0 && more inputs than outputs", totalInputs, totalOutputs);
  $.checkState(totalInputs > 0 && totalOutputs > 0 && totalInputs >= totalOutputs);
  //console.log("  checkState() totalInputs %s - totalOutputs %s <= Defaults.MAX_TX_FEE %s", totalInputs, totalOutputs, Defaults.MAX_TX_FEE);
  $.checkState(totalInputs - totalOutputs <= Defaults.MAX_TX_FEE);

  //console.log("TxProposal._buildTx() returning t: \n", t);
  return t;
};
...
TxProposal.prototype.getBitcoreTx = function() {
  ...
  //console.log("TxProposal.getBitcoreTx()");
  var t = this._buildTx();
...
TxProposal.prototype.getEstimatedFee = function() {
  ...
  //console.log("TxProposal.getEstimatedFee() this.feePerKb %s * this.getEstimatedSize() / 1000", this.feePerKb, this.getEstimatedSize());
  //console.log("  fee.toFixed(0): ", fee.toFixed(0));
...
TxProposal.prototype._addSignaturesToBitcoreTx = function(tx, signatures, xpub) {
  ...
  //console.log("TxProposal._addSignaturesToBitcoreTx()");
  ...
  _.each(signatures, function(signatureHex) {
    ...
    try {
      ...
      //console.log("about to input.addSignature()... ");
      tx.inputs[i].addSignature(tx, s);
      i++;
    } catch (e) {
      //console.log("  signing Error: " + JSON.stringify(e));
    };
  });

  if (i != tx.inputs.length) {
    //console.log("TxProposal._addSignaturesToBitcoreTx() i %d != tx.inputs.length %d", i, tx.inputs.length);
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  //console.log("TxProposal.sign() ");
  try {
    // Tests signatures are OK
    var tx = this.getBitcoreTx();
    //console.log("  _addSignaturesToBitcoreTx()");
    this._addSignaturesToBitcoreTx(tx, signatures, xpub);

    //console.log("  addAction() accept");
