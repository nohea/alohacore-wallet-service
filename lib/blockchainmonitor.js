...
BlockchainMonitor.prototype.start = function(opts, cb) {
  ...
  async.parallel([
    ...
    function(done) {
      self.explorers = {};
      var monitoredNetworks = ['livenet', 'testnet'];
      if(opts.blockchainExplorerOpts && opts.blockchainExplorerOpts.monitoredNetworks) {
        console.log("monitoredNetworks from config");
        monitoredNetworks = opts.blockchainExplorerOpts.monitoredNetworks;
      }
      monitoredNetworks = []; //override - disable live socket monitor
      _.map(monitoredNetworks, function(network) {
        var explorer;
        if (opts.blockchainExplorers) {
          ...
          if (opts.blockchainExplorerOpts && opts.blockchainExplorerOpts[network]) {
            console.log("using opts.blockchainExplorerOpts from config");
            config = opts.blockchainExplorerOpts[network];
          }
          else {
            console.log("No opts.blockchainExplorerOpts found - default config");
          }
...
BlockchainMonitor.prototype._initExplorer = function(network, explorer) {
  //log.info("BlockchainMonitor._initExplorer(): network " + network);
  var self = this;
