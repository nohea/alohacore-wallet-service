...
function WalletService() {
  ...
  //console.log("WalletService() messageBroker: ", messageBroker);
  this.lock = lock;
  this.storage = storage;
  this.blockchainExplorer = blockchainExplorer;
  this.blockchainExplorerOpts = blockchainExplorerOpts;
  this.altcoinOpts = altcoinOpts;
...
WalletService.initialize = function(opts, cb) {
  ...
  console.log("WalletService.initialize() opts: ", opts);
  
  opts = opts || {};
  lock = opts.lock || new Lock(opts.lockOpts);
  blockchainExplorer = opts.blockchainExplorer;
  blockchainExplorerOpts = opts.blockchainExplorerOpts;
  altcoinOpts = opts.altcoinOpts;
  if (opts.request)
  ...
  function initMessageBroker(cb) {
    //console.log("server initMessageBroker() opts: ", opts);
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  console.log("WalletService.createWallet()");
...
WalletService.prototype._getSigningKey = function(text, signature, pubKeys) {
  ...
  if(pubKeys) { console.log("WalletService._getSigningKey() pubKeys.length ", pubKeys.length) };
  return _.find(pubKeys, function(item) {
    console.log("_getSigningKey() ", text, signature, item.key);
...
WalletService.prototype._notify = function(type, data, opts, cb) {
  ...
  //console.log("WalletService._notify()");
...
WalletService.prototype._notifyTxProposalAction = function(type, txp, extraArgs, cb) {
  ...
  console.log("WalletService._notifyTxProposalAction()");
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['alcp', 'bit'], value.toLowerCase());
    },
  }];
...
WalletService.prototype.verifyMessageSignature = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    console.log("WalletService.verifyMessageSignature() isValid ", isValid);
...
WalletService.prototype._getBlockchainExplorer = function(network) {
  var opts = {};
  console.log("WalletService._getBlockchainExplorer(%s)", network);
  console.log("this.blockchainExplorerOpts: ", this.blockchainExplorerOpts);
  ...
  if (this.blockchainExplorerOpts && this.blockchainExplorerOpts[network]) {
      ...
      console.log("WalletService._getBlockchainExplorer(): using opts from this.blockchainExplorerOpts[" + network + "]");
  }
  else {
      console.log("WalletService._getBlockchainExplorer(): using default opts - !this.blockchainExplorerOpts");
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  bc.getUtxos(addresses, function(err, utxos) {
    if (err) {
      console.log("  err from bc: ", err);
      return cb(err);
    }
    
    var utxos = _.map(utxos, function(utxo) {
      console.log("_getUtxos()  utxo: " + JSON.stringify(utxo));
      var u = _.pick(utxo, ['txid', 'vout', 'address', 'scriptPubKey', 'amount', 'satoshis', 'confirmations', 'blockindex', 'height']);
      //console.log("  got height from explorer: ", u.height);
      u.confirmations = u.confirmations || (u.height - u.blockindex + 1) || 0;
      u.locked = false;
...
WalletService.prototype._getUtxosForCurrentWallet = function(addresses, cb) {
  var self = this;
  console.log("WalletService._getUtxosForCurrentWallet()");
  ...
  var broadcastSpentInputs, bcastChangeAddresses;
  var unconfirmedChangeAmountTotal;
  
  async.series([
    function(next) {
      console.log("  _getUtxosForCurrentWallet() async.series()");
      return next();
    },
    function(next) {
      if (_.isArray(addresses)) {
        allAddresses = addresses;
	console.log("  _getUtxosForCurrentWallet() addresses is array");
        return next();
      }
      self.storage.fetchAddresses(self.walletId, function(err, addresses) {
        allAddresses = addresses;
	console.log("  _getUtxosForCurrentWallet() fetchAddresses(%s) allAddresses.length: %d", self.walletId, allAddresses.length);
        return next();
      });
    },
    function(next) {
	...
	console.log("  _getUtxosForCurrentWallet() _getUtxos(addressStrs) got allUtxos.length: ", allUtxos.length);
        return next();
      });
    },
    function(next) {
	...
	console.log("  _getUtxosForCurrentWallet() getPendingTxs() txps RAW: ", txps);

	console.log("  _getUtxosForCurrentWallet() getPendingTxs() txps.length: ", txps.length);
        var lockedInputs = _.map(_.flatten(_.pluck(txps, 'inputs')), utxoKey);
        _.each(lockedInputs, function(input) {
          if (utxoIndex[input]) {
            console.log("  lockedInput: ", utxoIndex[input]);
            utxoIndex[input].locked = true;
          }
        });
	console.log("  lockedInputs.length: ", lockedInputs.length);
        return next();
      });
    },
    function(next) {
      ...
      // Additional: This list is from the mongodb txs collection.
      // This can report an output 'spent' but the block explorer
      // reports 'unspent' still
	...
	console.log("  _getUtxosForCurrentWallet() : fetchBroadcastedTxs() ");
	bcastChangeAddresses = _.map(_.pluck(txs, 'changeAddress'));
	console.log("  bcastChangeAddresses tx.changeAddress []: ", bcastChangeAddresses);
	var unconfirmedChangeAmountTotal = 0;
	_.each(txs, function(tx) {
	  if(tx.changeAddress && tx.changeAddress.amount) {
	    console.log("tx.changeAddress.amount: ", tx.changeAddress.amount);
	    unconfirmedChangeAmountTotal += tx.changeAddress.amount;
	  }
	  else {
	    console.log("no change amount for tx.changeAddress: ", tx.changeAddress);
	  }
	});
	console.log("unconfirmedChangeAmountTotal: ", unconfirmedChangeAmountTotal);
	
	console.log("self.storage.fetchBroadcastedTxs txs: ", txs);
        var spentInputs = _.map(_.flatten(_.pluck(txs, 'inputs')), utxoKey);
        _.each(spentInputs, function(input) {
          console.log("  broadcasted spent output (not counting): utxoIndex[%s]", input);
          if (utxoIndex[input]) {
            utxoIndex[input].spent = true;
          }
          console.log("  txs[input]: ", txs[input]);
        });
	console.log("  allUtxos.length: ", allUtxos.length);
        allUtxos = _.reject(allUtxos, {
          spent: true
        });
	broadcastSpentInputs = spentInputs;
	console.log("  spentInputs.length: ", spentInputs.length);
	console.log("  [unspent] allUtxos.length: ", allUtxos.length);

	// get broadcast txp txids, and check for those in the mempool
	var bcastTxids = _.map(_.flatten(_.pluck(txs, 'inputs')), utxoKey);
    ...
    function(next) {
      ...
      console.log("  addressToPath allUtxos.length: ", allUtxos.length);
      return next();
    },
  ], function(err) {
    return cb(err, allUtxos, broadcastSpentInputs, unconfirmedChangeAmountTotal);
  });
};
...
WalletService.prototype._totalizeUtxos = function(utxos, broadcastSpentOutputs) {
  console.log("WalletService._totalizeUtxos() ");
  var balance = {
    ...
    pendingAmount: _.sum(broadcastSpentOutputs, 'satoshis'),
  };
...
WalletService.prototype._getBalanceFromAddresses = function(addresses, cb) {
  var self = this;
  console.log("WalletService._getBalanceFromAddresses()");
  console.log("  addresses.length: ", addresses.length);

  self._getUtxosForCurrentWallet(addresses, function(err, utxos, broadcastSpentOutputs) {
    if (err) return cb(err);

    console.log("WalletService._getBalanceFromAddresses() result of _getUtxosForCurrentWallet()");
    console.log("  utxos.length: ", utxos.length);
    console.log("  broadcastSpentOutputs.length: ", broadcastSpentOutputs.length);
    var balance = self._totalizeUtxos(utxos, broadcastSpentOutputs);
    ...
    _.each(utxos, function(utxo) {
      console.log("byAddress[%s].amount === %s", utxo.address, utxo.satoshis);
...
WalletService.prototype.getSendMaxInfo = function(opts, cb) {
  ...
  if (_.isNumber(opts.feePerKb)) {
    if (opts.feePerKb < Defaults.MIN_FEE_PER_KB || opts.feePerKb > Defaults.MAX_FEE_PER_KB)
      return cb(new ClientError('Invalid fee per KB [' + opts.feePerKb + '] [sendMaxInfo]'));
  }
...
WalletService.prototype._sampleFeeLevels = function(network, points, cb) {
  ...
  console.log("WalletService._sampleFeeLevels() from blockchain explorer");
  ...
  bc.estimateFee(points, function(err, result) {
    ...
    console.log("bc.estimateFee() ", network);
    var failed = [];
...
WalletService.prototype.getFeeLevels = function(opts, cb) {
  ...
  console.log("WalletService.getFeeLevels() Returns fee levels for the current state of the network.");
  console.log("WalletService.getFeeLevels() opts ", JSON.stringify(opts));
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  console.log("WalletService._selectTxInputs() txpAmount ", txpAmount);
  console.log("WalletService._selectTxInputs() baseTxpSize ", baseTxpSize);
  console.log("WalletService._selectTxInputs() txp.feePerKb ", txp.feePerKb);
  console.log("WalletService._selectTxInputs() baseTxpFee ", baseTxpFee);
  console.log("WalletService._selectTxInputs() sizePerInput ", sizePerInput);
  console.log("WalletService._selectTxInputs() feePerInput ", feePerInput);
  ...
  function select(utxos, cb) {
    ...
    console.log("  select() totalValueInUtxos ", totalValueInUtxos);
    console.log("  select() netValueInUtxos %s = totalValueInUtxos %s - baseTxpFee %s - (utxos.length %d * feePerInput %s)", netValueInUtxos, totalValueInUtxos, baseTxpFee, utxos.length, feePerInput);

    var peercoinStyleFee = true; // TODO: make configurable // toFIX
    var fixedStyleFee;
    if(peercoinStyleFee) {
      // round up to granular fee increment
      var roundUpFees = baseTxpFee + (utxos.length * feePerInput);
      fixedStyleFee = peercoinRoundUpFees(roundUpFees, txp.feePerKb );
      console.log("peercoinStyleFee baseTxpFee round up ", baseTxpFee);
      console.log("  select() netValueInUtxos %s = totalValueInUtxos %s - roundup(baseTxpFee %s + (utxos.length %d * feePerInput %s))", netValueInUtxos, totalValueInUtxos, baseTxpFee, utxos.length, feePerInput);
      netValueInUtxos = totalValueInUtxos - roundUpFees;
      console.log("peercoinStyleFee netValueInUtxos ", netValueInUtxos);
    }
    
    function peercoinRoundUpFees(baseFeeSat, feePerKbSat) {
      console.log("peercoinRoundUpFees(" + baseFeeSat +", "+feePerKbSat+")");
      var feesCeil = (Math.ceil((baseFeeSat / feePerKbSat)) * (feePerKbSat / 1e8)) * 1e8;
      console.log("  feesCeil: " + feesCeil);
      return feesCeil;
    }
    ...
    if(peercoinStyleFee && fixedStyleFee) {
      fee = fixedStyleFee;
    }
...
WalletService.prototype._validateAndSanitizeTxOpts = function(wallet, opts, cb) {
  ...
  async.series([
    ...
    function(next) {
      ...
      if (_.isNumber(opts.feePerKb)) {
	console.log("Defaults.MIN_FEE_PER_KB: ", Defaults.MIN_FEE_PER_KB);
	console.log("Defaults.MAX_FEE_PER_KB: ", Defaults.MAX_FEE_PER_KB);
        if (opts.feePerKb < Defaults.MIN_FEE_PER_KB || opts.feePerKb > Defaults.MAX_FEE_PER_KB)
          return next(new ClientError('Invalid fee per KB [' + opts.feePerKb + '] [_validateAndSanitizeTxOpts]'));
      }
...
WalletService.prototype._getFeePerKb = function(wallet, opts, cb) {
  ...
  console.log("WalletService._getFeePerKb() opts.feePerKb ", opts.feePerKb);
    
  if (_.isNumber(opts.feePerKb)) return cb(null, opts.feePerKb);

  console.log("WalletService._getFeePerKb() from getFeeLevels()");
  self.getFeeLevels({
    network: wallet.network
  }, function(err, levels) {
    if (err) return cb(err);
...
WalletService.prototype._convertTxpSatoshiToCoinProtocol = function(opts, fromSatToCoin, toSatToCoin, cb) {
  var self = this;
  ...
  console.log("_convertTxpSatoshiToCoinProtocol() : coinRatio: ", coinRatio);
...
WalletService.prototype.createTx = function(opts, cb) {
  ...
  console.log("WalletService.createTx()");

  // the tx proposal is the last format before building the raw transaction,
  // so we are going to use the native COIN to "satoshi" ratio.
  // For Bitcoin it is COIN === 1e8 , for Peercoin, COIN === 1e6
  ...
  function checkTxpAlreadyExists(txProposalId, cb) {
    ...
    console.log("  checkTxpAlreadyExists() self.storage.fetchTx() ", self.walletId, txProposalId);
  ...
  self._runLocked(cb, function(cb) {
    ...
    self.getWallet({}, function(err, wallet) {
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          function(next) {
            ...
            self._getFeePerKb(wallet, opts, function(err, fee) {
              feePerKb = fee;
              console.log("createTx() feePerKb ", feePerKb);
              next();
            });
          },
          function(next) {
          ...
          function(next) {
	    //opts.satoshiToCoinProtocol = Defaults.COIN; // Bitcoin
	    opts.satoshiToCoinProtocol = Defaults.PEERCOIN_COIN; // Peercoin
            self._convertTxpSatoshiToCoinProtocol(txp, Defaults.COIN, opts.satoshiToCoinProtocol, next);
          },
          function(next) {
	    ...
	    console.log("  self.storage.storeTx(): wallet.id %s, txp.id %s", wallet.id, txp.id);
	    console.log("  if raw, txp stored would be: ", txp.getRawTx());
...
WalletService.prototype.publishTx = function(opts, cb) {
  ...
  console.log("WalletService.publishTx()");
  function utxoKey(utxo) {
    return utxo.txid + '|' + utxo.vout
  };
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      console.log("  self.storage.fetchTx() walletId, txProposalId: ", self.walletId, opts.txProposalId);
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        if (err) {
	  console.log("  fetchTx() err: ", err);
	  return cb(err);
	}
        if (!txp) {
	  console.log("  fetchTx() !txp not found");
	  return cb(Errors.TX_NOT_FOUND);
	}
        if (!txp.isTemporary()) {
	  console.log("  fetchTx() !txp.isTemporary()");
	  return cb(null, txp);
	}
	...
	console.log("  got raw tx: ", raw);
	console.log("  opts.proposalSignature: " + opts.proposalSignature);
	
        console.log("WalletService.publishTx() self._getSigningKey(raw, opts.proposalSignature, copayer.requestPubKeys");
	  ...
	  console.log("  !signingKey");
        ...
        self.getUtxos({}, function(err, utxos) {
          ...
          var unavailable = _.any(txpInputs, function(i) {
            ...
            if(!utxo || utxo.locked) {
              console.log("WalletService.publishTx() checking unavailable utxo: ", utxo);
            }
	  ...
	  console.log("  uxtos still available, txp.status = pending");
          txp.status = 'pending';
          self.storage.storeTx(self.walletId, txp, function(err) {
            if (err) return cb(err);

	    var raw = txp.getRawTx();
	    console.log("  stored txp: " + JSON.stringify(txp));
	    console.log("  raw tx expected: " + raw);
	    console.log("  self.storage.storeTx() done...");
...
WalletService.prototype.getTx = function(opts, cb) {
  ...
  console.log("WalletService.getTx(): ");
  console.log("  self.storage.fetchTx(): ", self.walletId, opts.txProposalId);
  self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
    if (err) return cb(err);
    ...
    console.log("  fetched txp: " + JSON.stringify(txp));
  ...
  self.storage.fetchPendingTxs(self.walletId, function(err, txps) {
    ...
    async.each(txps, function(txp, next) {
      ...
      self._checkTxInBlockchain(txp, function(err, isInBlockchain) {
        if (err) return next(err);
        if (!isInBlockchain) {
	  console.log("getPendingTxs() fetchPendingTxs() txp not in blockchain right now, id: ", txp.id);
	  return next(err);
	}
	
	console.log("getPendingTxs() fetchPendingTxs() txp _processBoradcast() ", txp.id);
        self._processBroadcast(txp, {
          byThirdParty: true
        }, next);
      });
	...
	console.log("getPendingTxs() fetchPendingTxs() txp status: %s, id: %s", txp.status, txp.id);
...
WalletService.prototype._normalizeTxHistory = function(txs) {
  console.log("WalletService._normalizeTxHistory()");
  var now = Math.floor(Date.now() / 1000);

  return _.map([].concat(txs), function(tx) {
    var inputs = _.map(tx.vin, function(item, index) {
      var vin = {
	address: item.addr,
	amount: item.valueSat,
      };
      
      console.log("  vin[%d] = ", index, vin);
      return vin;
    });

    var outputs = _.map(tx.vout, function(item, index) {
      var itemAddr;
    ...
    var fees = parseInt((tx.fees * 1e8).toFixed(0));
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function decorate(wallet, txs, addresses, proposals, notes) {
    ...
    return _.map(txs, function(tx) {
      ...
      console.log("WalletService.getTxHistory() _.map(txs) tx: " + JSON.stringify(tx));
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    console.log("WalletService.getTxHistory(): getNormalizedTxs()");
    ...
    async.series([
      ...
      function(next) {
        ...
        var bc = self._getBlockchainExplorer(network);
        console.log("WalletService.getTxHistory(): calling bc.getTransactions()");
        bc.getTransactions(addressStrs, from, to, function(err, rawTxs, total) {
          if (err) return next(err);

          //console.log("WalletService.getTxHistory(): rawTxs: " + JSON.stringify(rawTxs));
          console.log("WalletService.getTxHistory(): calling _normalizeTxHistory(rawTxs)");
          txs = self._normalizeTxHistory(rawTxs);
          totalItems = total;
          console.log("WalletService.getTxHistory(): totalItems: " + totalItems);
          return next();
        });
      },
      function(next) {
